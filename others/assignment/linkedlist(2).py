...
def insertNode(list, index, value):
    newnode = {'data':value}  # Create a new node and pass back the new node,
    newnode_sec = list
    if index == 0:  # against the invalid index here  # adding the node at teh beginning of the list
        ...
        index -= 1  # make the searching stop at before index for value inserting
        count = 0
        while count < index:
...
def switch(list, index):
    ...
    if result is None:
        ...
        newnode['data'] = result['next']['data']
        newnode['next'] = {'data': result['data'], 'next': None}
        newnode['next']['next'] = result['next']['next']
        count = 0
        while count < (index-2):
            list = list['next']
            count += 1
...
def testSwitch():
    ...
    '''
    myList = {'data': 10, 'next': {'data': 30, 'next': {'data': 20, 'next': {'data': 40, 'next': {'data': 50, 'next': {'data': 60, 'next': None}}}}}}
