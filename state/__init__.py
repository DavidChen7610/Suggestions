...
def stateful(cls):
    ...
    for i in cls.__dict__.values():
        if inspect.isclass(i) and issubclass(i, State) and hasattr(i, 'default') and i.default:
            defaults.append(i)
    if not defaults:
        raise Error('%s\'s default state is not found.' % cls.__name__)  # noqa
    if len(defaults) > 1:
        raise Error('%s\'s has too much default state.%s' % (cls.__name__, defaults))  # noqa
    default = defaults[0]
    ...
    def __getattr__(self, name):
        try:
            old__getattr__(self, name)  # 其实没有用到
